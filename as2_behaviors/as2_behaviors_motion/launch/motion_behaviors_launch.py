...
def get_node(context, behavior_name: str):
    """ Returns the behavior node """
    plugin_name = LaunchConfiguration(
        behavior_name + '_plugin_name').perform(context)
    if not plugin_name:
    ...
    behavior_config_file = LaunchConfiguration(
        behavior_name + '_config_file').perform(context)
...
def generate_launch_description():
    ...
    launch_description.append(DeclareLaunchArgument('namespace'))
    ...
    for behavior in behaviors:
        ...
        launch_description.append(
            DeclareLaunchArgument(behavior + '_config_file', default_value='',
                                  description='Path to behavior config file'))
        launch_description.append(
            OpaqueFunction(function=get_node, args=[behavior]))

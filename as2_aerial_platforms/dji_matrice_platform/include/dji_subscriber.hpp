 ...
 protected:
  ...
  void onUpdate() override {
    ...
    compass_msg_.header.frame_id =
        generateTfName(node_->get_namespace(), "imu");
...
class DJISubscriptionImu : public DJISubscription {
 ...
 protected:
  ...
  void onUpdate() override {
    ...
    imu_msg_.header.frame_id = generateTfName(node_->get_namespace(), "imu");
 ...
 protected:
  ...
  void onUpdate() override {
    ...
    if (!is_gps_initialized_) {
      ...
      gps_utils_.SetOrigin(gps.latitude * 180.0 / M_PI,
    ...
    double x, y, z;
    gps_utils_.LatLon2Local(gps.latitude * 180.0 / M_PI,
                            gps.longitude * 180.0 / M_PI, gps.altitude, x, y,
                            z);
    odom_msg_.header.stamp = node_->now();
    odom_msg_.header.frame_id = generateTfName(node_->get_namespace(), "odom");
    odom_msg_.child_frame_id =
        generateTfName(node_->get_namespace(), "base_link");
    ...
    as2::FrameUtils::quaternionToEuler(odom_msg_.pose.pose.orientation,
                                       roll_frd, pitch_frd, yaw_frd);
    ...
    as2::FrameUtils::eulerToQuaternion(roll_flu, pitch_flu, yaw_flu,
    ...
    auto flu_speed = as2::FrameUtils::convertENUtoFLU(
        odom_msg_.pose.pose.orientation, vel_ENU);

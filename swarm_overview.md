...
- **출력**: 드론별 배정(`/{drone}/swarm_agent/task`: 종류/수단/역할/경로/구역).
...
- **입력**: AlertEvent (지오펜스 위반/충돌위험/배터리 등).
- **출력**: 비상 비행 (Emergency).
...
- **입력**: heartbeat 상태, 캐시된 배정.
- **출력**: 자율 비행(AutonomousCached) 또는 임무 복귀.
- **연관**: ← F1(상실 감지), ← F3(캐시 배정 사용).
...
8. 종료      coverage/배터리/시간 → 리더 온보드 판정            (F3 내 종료조건)
9. 복귀·보고 RTB → 착륙 → 보고서 → GCS                          (F6,F10)

※ 어느 단계든: 비상(F8) 선점, 리더/GCS 단절(F9) 자율 지속.
```
...
> 상세 구현: `swarm_bt_native_design.md`(설계), `swarm_bt_node_reference.md`(노드 IO), `swarm_mission_example.xml`(트리), `swarm_intent_processing.md`(의도 처리). 미해결: `swarm_open_issues.md`.
</content>

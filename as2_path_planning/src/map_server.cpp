...
void MapServer::clearOccGrid()
{
  ...
  occ_grid_->data.assign(
    occ_grid_->info.width * occ_grid_->info.height,
    -1);                      // unknown
}
  ...
  const as2_msgs::msg::LabeledOccupancyGrid::SharedPtr msg)
{
  ...
  if (grid_map_.exists(msg->label)) {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Creating new layer: %s",
      msg->label.c_str());
  }
...
static void printMatInfo(const rclcpp::Node * node, cv::Mat mat)
{
  ...
  if (mat.channels() == 1) {
    ...
    RCLCPP_INFO(
      node->get_logger(), "Map max value %f at (%d, %d)", maxVal,
      maxLoc.x, maxLoc.y);
    RCLCPP_INFO(
      node->get_logger(), "Map min value %f at (%d, %d)", minVal,
      minLoc.x, minLoc.y);
  }
}

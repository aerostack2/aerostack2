...
: Node("path_planner", rclcpp::NodeOptions().use_intra_process_comms(true))
{
  ...
  navigation_action_server_ = rclcpp_action::create_server<NavigateToPoint>(
    ...
    std::bind(
      &PathPlanner::navigationGoalCbk, this, std::placeholders::_1,
      std::placeholders::_2),
    std::bind(&PathPlanner::navigationCancelCbk, this, std::placeholders::_1),
    std::bind(
      &PathPlanner::navigationAcceptedCbk, this,
      std::placeholders::_1));
...
{
  RCLCPP_INFO(
    this->get_logger(), "Received goal request with point (%f, %f)",
    goal->point.point.x, goal->point.point.y);

  // World to image transformations
  RCLCPP_INFO(this->get_logger(), "frame id: %s", last_occ_grid_.header.frame_id.c_str());
  cv::Point2i goal_px =
    utils::pointToPixel(
  ...
  cv::Point2i origin =
    utils::pointToPixel(
    drone_pose_, last_occ_grid_.info,
    last_occ_grid_.header.frame_id, tf_buffer_);

  // Erode obstacles
  cv::Mat mat = utils::gridToImg(last_occ_grid_);

  int iterations = std::ceil(
    safety_distance_ /
  ...
  auto test = utils::imgToGrid(
    mat, last_occ_grid_.header,
  ...
  const std::shared_ptr<GoalHandleNavigateToPoint> goal_handle)
{
  ...
  if (path_.empty()) {
    RCLCPP_ERROR(this->get_logger(), "Empty path. Aborting navigation.");
  ...
  // {
  //   RCLCPP_ERROR(
  //     this->get_logger(), "Follow Path Action server not available "
  ...
  //   std::bind(
  //   &PathPlanner::followPathFeedbackCbk, this,

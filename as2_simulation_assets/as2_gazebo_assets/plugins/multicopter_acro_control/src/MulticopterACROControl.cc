...
#include "MulticopterACROControl.hh"
...
//////////////////////////////////////////////////
void MulticopterACROControl::Configure(
  ...
  EventManager & /*_eventMgr*/)
{
  ...
  if (!this->model.Valid(_ecm)) {
    gzerr << "MulticopterACROControl plugin should be attached to a model "
  ...
  if (sdfClone->HasElement("rotorConfiguration")) {
    vehicleParams.rotorConfiguration =
      ...
      _ecm, sdfClone->GetElement("rotorConfiguration"),
  ...
  if (sdfClone->HasElement("velocityGain")) {
    ...
    gzerr << "Please specify velocityGain for MulticopterACROControl.\n";
    return;
  }
  ...
  if (sdfClone->HasElement("attitudeGain")) {
    ...
    gzerr << "Please specify attirudeGain for MulticopterACROControl.\n";
    return;
  }
  ...
  if (sdfClone->HasElement("angularRateGain")) {
    ...
    gzerr << "Please specify angularRateGain MulticopterACROControl.\n";
    return;
  }
  ...
  this->node.Subscribe(topic, &MulticopterACROControl::OnACRO, this);
  gzmsg << "MulticopterACROControl subscribing to ACRO messages on ["
        << topic << "]" << std::endl;

  std::string enableTopic{this->robotNamespace + "/" + this->enableSubTopic};
  this->node.Subscribe(
    enableTopic, &MulticopterACROControl::OnEnable,
    this);
  gzmsg << "MulticopterACROControl subscribing to Boolean messages on ["
...
//////////////////////////////////////////////////
math::Inertiald MulticopterACROControl::VehicleInertial(
...
//////////////////////////////////////////////////
void MulticopterACROControl::PreUpdate(
  const UpdateInfo & _info,
  EntityComponentManager & _ecm)
{
  GZ_PROFILE("MulticopterACROControl::PreUpdate");
  ...
  if (!this->controllerActive) {
    ...
    if (this->rotorVelocities.squaredNorm() > 0) {
      ...
      // Clear the acroMsg so that the system waits for a new command after
      // being renabled.
      std::lock_guard<std::mutex> lock(this->acroMsgMutex);
      this->acroMsg.reset();
    }
    return;
  }
  ...
  {
    std::lock_guard<std::mutex> lock(this->acroMsgMutex);
    if (!this->acroMsg.has_value()) {
      return;
    }
    acro.rollRate = this->acroMsg->x();
    acro.pitchRate = this->acroMsg->y();
    acro.yawRate = this->acroMsg->z();
    acro.thrust = this->acroMsg->w();
  }
...
//////////////////////////////////////////////////
void MulticopterACROControl::OnACRO(
...
//////////////////////////////////////////////////
void MulticopterACROControl::OnEnable(
...
//////////////////////////////////////////////////
void MulticopterACROControl::PublishRotorVelocities(
  EntityComponentManager & _ecm,
...
GZ_ADD_PLUGIN(
  MulticopterACROControl,
  gz::sim::System,
  MulticopterACROControl::ISystemConfigure,
  MulticopterACROControl::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(
  MulticopterACROControl,
  "gz::sim::systems::MulticopterACROControl")

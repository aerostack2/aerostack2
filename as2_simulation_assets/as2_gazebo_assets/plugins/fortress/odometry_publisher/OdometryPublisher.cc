...
//////////////////////////////////////////////////
gz::sim::systems::OdometryPublisher::OdometryPublisher()
: dataPtr(std::make_unique<OdometryPublisherPrivate>())
{
}

//////////////////////////////////////////////////
void gz::sim::systems::OdometryPublisher::Configure(
...
//////////////////////////////////////////////////
void gz::sim::systems::OdometryPublisher::PreUpdate(
  const gz::sim::UpdateInfo & _info,
  gz::sim::EntityComponentManager & _ecm)
{
  ...
  if (!this->dataPtr->model.Valid(_ecm)) {
    ignwarn << "OdometryPublisher model no longer valid. "
...
//////////////////////////////////////////////////
void gz::sim::systems::OdometryPublisher::PostUpdate(
...
//////////////////////////////////////////////////
void gz::sim::systems::OdometryPublisherPrivate::UpdateOdometry(
  const gz::sim::UpdateInfo & _info,
  const gz::sim::EntityComponentManager & _ecm)
{
  ...
  math::Vector3d angVelRaw = angularVelComp->Data();
  ...
  math::Vector3d angVel(
    (this->dimensions == 3) ? angVelRaw.X() + math::Rand::DblNormal(0.0, angNoise) : 0.0,
    (this->dimensions == 3) ? angVelRaw.Y() + math::Rand::DblNormal(0.0, angNoise) : 0.0,
    angVelRaw.Z() + math::Rand::DblNormal(0.0, angNoise)
  );
  ...
  if (omegaMag > 1e-10) {
    ...
    q_delta = math::Quaterniond(axis, angle);
  }
  ...
  // Set timestamp
  msgs::Header header;
  auto simTimeNs = std::chrono::duration_cast<std::chrono::nanoseconds>(_info.simTime).count();
  header.mutable_stamp()->set_sec(simTimeNs / 1000000000);
  header.mutable_stamp()->set_nsec(simTimeNs % 1000000000);
...
IGNITION_ADD_PLUGIN(
  ...
  System,
  gz::sim::systems::OdometryPublisher::ISystemConfigure,
  gz::sim::systems::OdometryPublisher::ISystemPreUpdate,
  gz::sim::systems::OdometryPublisher::ISystemPostUpdate)

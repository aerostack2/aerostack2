...
### M0-1. 개별 드론 부팅 및 네트워크 참여

각 드론은 독립적인 비행체로 부팅됨.

초기 상태: BOOT -> PRECHECK -> READY
...
이후 DDS 네트워크 또는 Mesh 네트워크에 참여함.


### M0-2. 멤버 발견 (Presence Discovery)

각 드론은 Heartbeat 발행을 통해 주기적으로 자신의 존재를 알림.

```
vehicle_id: {unitcode}  # 예: uav00001, ugv00001
swarm_id: unknown
state: READY
payload:
...
위치, 배터리 등 드론 정보는 별도의 토픽으로 관리

Heartbeat 주기: 1Hz ~ 5Hz
...
GCS에서 /global/heartbeat 를 통해 구성한 uav 목록을 이용하여 군집 생성 명령.

```
swarm_id: 1
leader_id: null or {unitcode}
member_list: [unicode, ...]
```

/{unitcode}/swarm/create

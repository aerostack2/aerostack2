...
void PathDisplay::onInitialize(const DisplayContext &context) {
  ...
  line_width_ =
      static_cast<float>(getOrDeclare<double>(node, ns + "line_width", 0.05));
  ...
  line_ =
      std::make_unique<rviz_rendering::BillboardLine>(scene_manager_, node_);
  line_->setLineWidth(line_width_);
  line_->setColor(static_cast<float>(color_v_.size() > 0 ? color_v_[0] : 0.1),
  ...
  auto cb = [this](nav_msgs::msg::Path::ConstSharedPtr msg) {
...
void PathDisplay::update(const rclcpp::Time & /*stamp*/,
...
PLUGINLIB_EXPORT_CLASS(as2_camera_overlay::PathDisplay,

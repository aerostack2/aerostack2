#include "displays/grid_display.hpp"
...
void GridDisplay::onInitialize(const DisplayContext &context) {
  ...
  grid_ = std::make_unique<rviz_rendering::Grid>(
      context.scene_manager, node_, rviz_rendering::Grid::Lines,
      static_cast<uint32_t>(cell_count), static_cast<float>(cell_size),
      static_cast<float>(line_width), color);
  grid_->create();
}

void GridDisplay::update(const rclcpp::Time & /*stamp*/,
  ...
  if (reference_frame_.empty() || reference_frame_ == fixed_frame) {
  ...
  } else if (!lookupTransformOgre(*tf_buffer_, fixed_frame, reference_frame_,
    ...
    RCLCPP_WARN_THROTTLE(logger_, *rclcpp::Clock::make_shared(), 5000,
                         ...
                         fixed_frame.c_str(), reference_frame_.c_str(),
                         error.c_str());
    return;
  }
...
PLUGINLIB_EXPORT_CLASS(as2_camera_overlay::GridDisplay,

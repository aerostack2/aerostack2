...
void MarkerArrayDisplay::onInitialize(const DisplayContext &context) {
  ...
  for (const auto &topic : topics) {
    auto cb = [this](MarkerArrayMsg::ConstSharedPtr msg) {
...
void MarkerArrayDisplay::update(const rclcpp::Time &,
...
bool MarkerArrayDisplay::computeWorldPose(const MarkerMsg &marker,
                                          const std::string &fixed_frame,
  ...
  std::string err;
  if (!lookupTransformOgre(*tf_buffer_, fixed_frame, marker.header.frame_id,
                           rclcpp::Time(0, 0), frame_pos, frame_rot, &err)) {
    RCLCPP_WARN_THROTTLE(logger_, *rclcpp::Clock::make_shared(), 5000,
                         "MarkerArrayDisplay: TF lookup %s -> %s failed: %s",
                         fixed_frame.c_str(), marker.header.frame_id.c_str(),
                         err.c_str());
    return false;
  }
...
void MarkerArrayDisplay::applyMarker(const MarkerMsg &marker,
                                     const std::string &fixed_frame) {
  ...
  if (!type_changed && !points_changed) {
    ...
    case MarkerMsg::CYLINDER:
      if (mn.shapes.size() == 1) {
        ...
        mn.shapes[0]->setScale(
            Ogre::Vector3(static_cast<float>(marker.scale.x),
...
void MarkerArrayDisplay::buildShape(const MarkerMsg &marker, MarkerNode &node,
                                    const Ogre::Vector3 &world_pos,
                                    const Ogre::Quaternion &world_rot) {
  rviz_rendering::Shape::Type type = rviz_rendering::Shape::Cube;
  switch (marker.type) {
  case MarkerMsg::SPHERE:
    type = rviz_rendering::Shape::Sphere;
    break;
  case MarkerMsg::CYLINDER:
    type = rviz_rendering::Shape::Cylinder;
    break;
  case MarkerMsg::CUBE:
  case MarkerMsg::MESH_RESOURCE:
  default:
    type = rviz_rendering::Shape::Cube;
    break;
  }
  auto shape = std::make_unique<rviz_rendering::Shape>(type, scene_manager_,
                                                       node.scene_node);
  shape->setPosition(world_pos);
  shape->setOrientation(world_rot);
  shape->setScale(Ogre::Vector3(static_cast<float>(marker.scale.x),
                                static_cast<float>(marker.scale.y),
                                static_cast<float>(marker.scale.z)));
...
void MarkerArrayDisplay::buildArrow(const MarkerMsg &marker, MarkerNode &node,
                                    const Ogre::Vector3 &world_pos,
                                    const Ogre::Quaternion &world_rot) {
  auto arrow = std::make_unique<rviz_rendering::Arrow>(
      scene_manager_, node.scene_node, static_cast<float>(marker.scale.x * 0.8),
      static_cast<float>(marker.scale.y),
      static_cast<float>(marker.scale.x * 0.2),
      static_cast<float>(marker.scale.z));
...
void MarkerArrayDisplay::buildLineList(const MarkerMsg &marker,
                                       MarkerNode &node,
                                       const Ogre::Vector3 &world_pos,
                                       const Ogre::Quaternion &world_rot,
                                       bool strip) {
  ...
  auto line = std::make_unique<rviz_rendering::BillboardLine>(scene_manager_,
                                                              node.scene_node);
  line->setLineWidth(static_cast<float>(std::max(marker.scale.x, 0.01)));
  line->setColor(marker.color.r, marker.color.g, marker.color.b,
                 marker.color.a);
  ...
  if (strip) {
    ...
    for (size_t i = 0; i < pairs; ++i) {
      const Ogre::Vector3 a =
          world_pos + world_rot * toOgreVector(marker.points[2 * i]);
      const Ogre::Vector3 b =
          world_pos + world_rot * toOgreVector(marker.points[2 * i + 1]);
      line->addPoint(a);
      line->addPoint(b);
      line->finishLine();
    }
...
void MarkerArrayDisplay::buildPoints(const MarkerMsg &marker, MarkerNode &node,
                                     const Ogre::Vector3 &world_pos,
                                     const Ogre::Quaternion &world_rot) {
...
void MarkerArrayDisplay::buildList(const MarkerMsg &marker, MarkerNode &node,
                                   const Ogre::Vector3 &world_pos,
                                   const Ogre::Quaternion &world_rot) {
...
void MarkerArrayDisplay::buildMesh(const MarkerMsg &marker, MarkerNode &node,
                                   const Ogre::Vector3 &world_pos,
                                   const Ogre::Quaternion &world_rot) {
  if (marker.mesh_resource.empty()) {
    RCLCPP_WARN_THROTTLE(logger_, *rclcpp::Clock::make_shared(), 10000,
                         "MarkerArrayDisplay: MESH_RESOURCE marker has empty "
                         "mesh_resource (ns=%s id=%d)",
                         marker.ns.c_str(), marker.id);
    return;
  }

  if (node.entity != nullptr && node.mesh_resource == marker.mesh_resource &&
    ...
    node.scene_node->setScale(Ogre::Vector3(
        static_cast<float>(marker.scale.x), static_cast<float>(marker.scale.y),
        static_cast<float>(marker.scale.z)));
    return;
  }

  Ogre::MeshPtr mesh =
      rviz_rendering::loadMeshFromResource(marker.mesh_resource);
  if (!mesh) {
    RCLCPP_WARN_THROTTLE(
        logger_, *rclcpp::Clock::make_shared(), 10000,
  ...
  Ogre::MaterialPtr default_mat =
      rviz_rendering::MaterialManager::createMaterialWithLighting(entity_id +
                                                                  "Material");
  ...
  node.scene_node->setPosition(world_pos);
  node.scene_node->setOrientation(world_rot);
  node.scene_node->setScale(Ogre::Vector3(static_cast<float>(marker.scale.x),
                                          static_cast<float>(marker.scale.y),
                                          static_cast<float>(marker.scale.z)));
}

void MarkerArrayDisplay::buildText(const MarkerMsg &marker, MarkerNode &node,
                                   const Ogre::Vector3 &world_pos) {
...
PLUGINLIB_EXPORT_CLASS(as2_camera_overlay::MarkerArrayDisplay,

...
void PoseArrayDisplay::onInitialize(const DisplayContext &context) {
  ...
  sub_ =
      node->create_subscription<geometry_msgs::msg::PoseArray>(topic, 10, cb);
}

void PoseArrayDisplay::topicCallback(
...
void PoseArrayDisplay::rebuildShapes(size_t n) {
  if (shape_ == "axes") {
    while (axes_.size() < n) {
      axes_.emplace_back(std::make_unique<rviz_rendering::Axes>(
          scene_manager_, node_, length_, length_ * 0.1f));
    }
    while (axes_.size() > n) {
      axes_.pop_back();
    }
    ...
    while (arrows_.size() < n) {
      auto arrow = std::make_unique<rviz_rendering::Arrow>(
          scene_manager_, node_, length_ * 0.7f, 0.03f, length_ * 0.3f, 0.07f);
      arrow->setColor(
...
void PoseArrayDisplay::update(const rclcpp::Time & /*stamp*/,
...
PLUGINLIB_EXPORT_CLASS(as2_camera_overlay::PoseArrayDisplay,

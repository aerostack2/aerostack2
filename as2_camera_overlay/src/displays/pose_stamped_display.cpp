#include "displays/pose_stamped_display.hpp"
...
void PoseStampedDisplay::onInitialize(const DisplayContext &context) {
  ...
  if (shape_ == "axes") {
    ...
    arrow_ = std::make_unique<rviz_rendering::Arrow>(
        context.scene_manager, node_, static_cast<float>(length * 0.7), 0.05f,
        static_cast<float>(length * 0.3), 0.1f);
    arrow_->setColor(static_cast<float>(color_v.size() > 0 ? color_v[0] : 1.0),
  ...
  sub_ =
      node->create_subscription<geometry_msgs::msg::PoseStamped>(topic, 10, cb);
}

void PoseStampedDisplay::topicCallback(
    geometry_msgs::msg::PoseStamped::ConstSharedPtr msg) {
  std::lock_guard<std::mutex> lk(mutex_);
  last_msg_ = std::move(msg);
}

void PoseStampedDisplay::update(const rclcpp::Time &,
  ...
  std::string err;
  if (!lookupTransformOgre(*tf_buffer_, fixed_frame, msg->header.frame_id,
...
PLUGINLIB_EXPORT_CLASS(as2_camera_overlay::PoseStampedDisplay,

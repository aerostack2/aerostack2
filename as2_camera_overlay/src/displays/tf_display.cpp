#include "as2_camera_overlay/displays/tf_display.hpp"
...
void TFDisplay::onInitialize(const DisplayContext & context)
{
  ...
  axis_length_ = static_cast<float>(getOrDeclare<double>(node, ns + "length", 0.5));
  axis_radius_ = static_cast<float>(getOrDeclare<double>(node, ns + "radius", 0.05));
...
void TFDisplay::update(const rclcpp::Time &, const std::string & fixed_frame)
{
  ...
  for (auto & entry : frames_) {
    ...
    if (!lookupTransformOgre(
        *tf_buffer_, fixed_frame, entry.frame_id, rclcpp::Time(0, 0),
...
PLUGINLIB_EXPORT_CLASS(as2_camera_overlay::TFDisplay, as2_camera_overlay::OverlayDisplayBase)

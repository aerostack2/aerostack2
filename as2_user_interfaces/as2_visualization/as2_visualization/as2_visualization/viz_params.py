...
class TopicParams:
    ...
    def __post_init__(self):
        if self.history not in HISTORY_POLICIES:
            raise ValueError(
                f"{self.history} history policy not available {HISTORY_POLICIES}"
            )
        if self.reliability not in RELIABILITY_POLICIES:
            raise ValueError(
                f"{self.reliability} reliability policy not available {RELIABILITY_POLICIES}"
            )
        if self.durability not in DURABILITY_POLICIES:
            raise ValueError(
                f"{self.durability} durability policy not available {DURABILITY_POLICIES}"
            )
